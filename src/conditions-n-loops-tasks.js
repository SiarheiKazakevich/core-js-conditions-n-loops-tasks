...
function convertToRomanNumerals(num) {
  if (num < 1 || num > 39) {
    throw new Error('Number out of range (1 to 39)');
  }
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < middle; i += 1) {
    ...
    const rightChar = str[length - 1 - i];
...
function rotateMatrix(matrix) {
  ...
  while (left < right) {
    for (let i = 0; i < right - left; i += 1) {
      ...
      squareMatrix[top][left + i] = matrix[bottom - i][left];
      squareMatrix[bottom - i][left] = matrix[bottom][right - i];
      squareMatrix[bottom][right - i] = matrix[top + i][right];
